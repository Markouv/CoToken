...
def main(ckpt_dir: str, tokenizer_path: str, input_file: str = None, lr: float = 1e-3, num_epochs: int = 20, dataset: str = "CoToken", log_prefix="", only_functoken=False, log_each=False):
    ...
    func_dict_path = f"data/{dataset}/CoT_dict.json"
    ...
    funcmodel = load(ckpt_dir, tokenizer_path, local_rank=-1, world_size=-1, func_dict=func_dict)
    # print(funcmodel)
    ...
    for epoch in range(num_epochs):
        ...
        for case_idx, prompt in tqdm(enumerate(trainset)):
            ...
            print(f"loss:{loss}, result:{result}")
            loss.backward()
            optimizer.step()
