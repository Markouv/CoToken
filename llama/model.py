...
class FunctionLM(nn.Module):
    def __init__(self, base_model, tokenizer, func_dict, load_path=None, inference_mode="func_embedding"):
        ...
        # self.func_embed = nn.Linear(4096, len(func_dict), bias=False).to("cuda")
    ...
    def get_loss(self, raw_inputs, only_functoken=False):
        ...
        with torch.no_grad():
            ...
            # raw_input_ids = torch.tensor(self.tokenizer.encode(raw_inputs["text"], add_special_tokens=True))[:]
            # labels = torch.tensor(self.tokenizer.encode(raw_inputs["text"], add_special_tokens=True))[:]

            if "tar_output" not in raw_inputs:
                raw_inputs["tar_token"] = ["<" + raw_inputs["api"] + ">"]

            for s, t, eq in zip(raw_inputs["start_token_idx"], raw_inputs["end_token_idx"], raw_inputs["tar_token"]):
